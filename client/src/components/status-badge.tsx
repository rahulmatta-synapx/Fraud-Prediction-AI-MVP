...
export function StatusBadge({ status }: StatusBadgeProps) {
  const config: Record<ClaimStatus, { icon: React.ComponentType<{ className?: string }>; className: string }> = {
    needs_review: {
      icon: Clock,
  ...
  const { icon: Icon, className } = config[normalizedStatus] || config.needs_review;
  const label = getStatusDisplay(normalizedStatus);
